...
const nextImageExportOptimizer = async function () {
    ...
    // Default values
    let imageFolderPath = "public/img";
    ...
    for (const filename of allFilesInRemoteImageFolder) {
        ...
        console.log(`Deleted ${filename} from remote image folder as it is not retrieved from remoteOptimizedImages.js.`);
    }
    ...
    const allImagesInImageFolder = allFilesInImageFolderAndSubdirectories.filter((fileObject) => {
        ...
        if (filenameSplit.length === 1)
            return false;
        const extension = filenameSplit.pop().toLowerCase();
    ...
    for (let index = 0; index < allImagesInImageFolder.length; index++) {
        ...
        try {
            ...
            for (let indexWidth = 0; indexWidth < widths.length; indexWidth++) {
                ...
                // for a static image, we copy the image to public/img/optimized or public/${exportFolderName}
                ...
                const basePathToStoreOptimizedImages = isStaticImage ||
                    ...
                    ? "public"
                    : basePath;
                const optimizedFileNameAndPath = path.join(basePathToStoreOptimizedImages, fileDirectory, exportFolderName, `${filename}-opt-${width}.${extension.toLowerCase()}`);
